...
public class BPlusTree {
    ...
    public Optional<RecordId> get(DataBox key) {
        ...
        for (int i = 0; i < leafNode.getKeys().size(); i++) {
            if (leafNode.getKeys().get(i).getInt() == key.getInt()) {
                index = i;
            }
        }
        if (index != -1) return Optional.of(leafNode.getRids().get(index));
    ...
    public Iterator<RecordId> scanAll() {
        ...
        BPlusTreeIterator bPlusTreeIterator = new BPlusTreeIterator(root);
        // TODO(proj4_part3): B+ tree locking
    ...
    public Iterator<RecordId> scanGreaterEqual(DataBox key) {
        typecheck(key);
        // TODO(proj2): Return a BPlusTreeIterator.
        BPlusTreeIterator bPlusTreeIterator = new BPlusTreeIterator(root, key);
        // TODO(proj4_part3): B+ tree locking
    ...
    public void put(DataBox key, RecordId rid) {
        typecheck(key);
        // TODO(proj2): implement
        Optional<Pair<DataBox, Long>> put = root.put(key, rid);
        if (put.isPresent()) {
            List<DataBox> newKeys = Collections.singletonList(put.get().getFirst());
            List<Long> newChildren = Arrays.asList(root.getPage().getPageNum(), put.get().getSecond());
    ...
    public void bulkLoad(Iterator<Pair<DataBox, RecordId>> data, float fillFactor) {
        ...
        if (root instanceof LeafNode) {
            if (((LeafNode) root).getKeys().size() > 0) throw new BPlusTreeException("Tree should be empty");
        }
        int leafNodeSize = (int) Math.ceil(2 * this.metadata.getOrder() * fillFactor);
        List<Pair<DataBox, RecordId>> dataToBulkLoad = new ArrayList<>();
        data.forEachRemaining(dataToBulkLoad::add);

        List<List<Pair<DataBox, RecordId>>> dataInChunks = new ArrayList<>();
        List<Pair<DataBox, RecordId>> tempList = new ArrayList<>();
        for (Pair<DataBox, RecordId> dataBoxRecordIdPair : dataToBulkLoad) {
            tempList.add(dataBoxRecordIdPair);
            if (tempList.size() % (leafNodeSize) == 0) {
                dataInChunks.add(tempList);
        ...
        Optional<Pair<DataBox, Long>> boxLongPairRoot = root.bulkLoad(dataInChunks.get(0).iterator(), fillFactor);
        LeafNode leafNode = new LeafNode(metadata, bufferManager, Collections.emptyList(), Collections.emptyList(), Optional.empty(), lockContext);
        Optional<Pair<DataBox, Long>> boxLongPair = leafNode.bulkLoad(dataInChunks.get(1).iterator(), fillFactor);
        updateRoot(new InnerNode(metadata, bufferManager, Collections.singletonList(boxLongPair.get().getFirst()),
                Arrays.asList(boxLongPairRoot.get().getSecond(), boxLongPair.get().getSecond()), lockContext));

        for (int i = 2; i < dataInChunks.size(); i++) {
            Optional<Pair<DataBox, Long>> bulkLoad = root.bulkLoad(dataInChunks.get(i).iterator(), fillFactor);
            if (bulkLoad.isPresent()) {
                List<DataBox> newKeys = Collections.singletonList(bulkLoad.get().getFirst());
                List<Long> newChildren = Arrays.asList(root.getPage().getPageNum(), bulkLoad.get().getSecond());
                InnerNode newRoot = new InnerNode(metadata, bufferManager, newKeys, newChildren, lockContext);
                updateRoot(newRoot);
            }
        }
    ...
    private class BPlusTreeIterator implements Iterator<RecordId> {
        ...
        private BPlusTreeIterator(BPlusNode root) {
            ...
            recordIdIterator = currentLeafNode.getRids().iterator();
        }

        private BPlusTreeIterator(BPlusNode root, DataBox key) {
        ...
        public boolean hasNext() {
            // TODO(proj2): implement
            if (currentLeafNode != null && recordIdIterator.hasNext()) {
                return true;
                ...
                if (rightSibling.isPresent()) {
                    ...
                    this.recordIdIterator = currentLeafNode.getRids().iterator();
                    return this.recordIdIterator.hasNext();
                } else {
        ...
        public RecordId next() {
            // TODO(proj2): implement
            if (currentLeafNode == null) throw new NoSuchElementException();
            return recordIdIterator.next();
        }

        private void setUpIterator(DataBox key) {
            List<RecordId> rids = currentLeafNode.getRids();
            List<DataBox> dataBoxes = currentLeafNode.getKeys();
            List<RecordId> candidateRids = new ArrayList<>();
            for (int i = 0; i < dataBoxes.size(); i++) {
                if (dataBoxes.get(i).getInt() >= key.getInt()) {
                    candidateRids.add(rids.get(i));
                }
            }
            if (candidateRids.isEmpty()) {
                Optional<LeafNode> rightSibling = currentLeafNode.getRightSibling();
                if (rightSibling.isPresent()) {
                    currentLeafNode = rightSibling.get();
                    this.recordIdIterator = currentLeafNode.getRids().iterator();
                ...
                this.recordIdIterator = candidateRids.iterator();
            }
        }
    }
}
